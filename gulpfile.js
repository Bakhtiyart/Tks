...
if (gulpVersion == 4) {
	...
	gulp.task('watch', function() {
		...
		gulp.watch('app/libs/bootstrap/scss/**/*.scss', gulp.parallel('bootstrap'));
		gulp.watch(['libs/**/*.js', 'app/js/common.js'], gulp.parallel('scripts'));
		gulp.watch('app/*.html', gulp.parallel('code'));
		gmWatch && gulp.watch('app/img/_src/**/*', gulp.parallel('img')); // GraphicsMagick watching image sources if allowed.
	});
	gmWatch ? gulp.task('default', gulp.parallel('img', 'styles', 'bootstrap', 'scripts', 'browser-sync', 'watch')) 
					: gulp.task('default', gulp.parallel('styles', 'bootstrap', 'scripts', 'browser-sync', 'watch'));
